...
const ENHANCEMENT_PROMPT = `Use the uploaded image as the exact new Fakemon design reference for the output.
Recreate the same Fakemon, keeping the body structure, pose, key features intact, and same color palette.
...
Do not change the Fakemon into a different one, and do not change its overall body orientation.
